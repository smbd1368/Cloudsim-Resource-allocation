...
class HeterogeneousCloudSimulator:
    ...
    def create_and_add_task(self, workflow, cpu_req=None, gpu_req=None, fpga_req=None, length=None, priority=None):
        self.task_id_counter += 1
        if cpu_req is None:
            cpu_req = random.uniform(0.1, 2.7)
        if gpu_req is None:
    ...
    def run_simulation(self):
        ...
        print(total_tasks, 'tasks to process...')
        completed_tasks = 0
        ...
        while self.workflows:
            for workflow in self.workflows[:]:
                if workflow.tasks:
                    ...
                    workflow.tasks.sort(key=lambda t: t.priority, reverse=True)
    ...
    def can_allocate_task(self, vm, task):
        return vm.can_host(task)

    def visualize_workflows(self):
        # Create a graph to visualize task dependencies
        G = nx.DiGraph()
        ...
        for workflow in self.workflows:
            for task in workflow.tasks:
                ...
                for dependency in task.dependencies:
                    G.add_edge(dependency.id, task.id)
